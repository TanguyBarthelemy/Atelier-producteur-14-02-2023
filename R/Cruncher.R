...
################################################################################
#################            Programmes du cruncher            #################
################################################################################

# Import et chargement des packages --------------------------------------------
...
# attention : policy "complete" = casse tout



# si besoin
# liste WS = liste des fichiers xml du repertoire
# liste_f_xml_maitre <- list.files(pattern = "\\.xml$")
# liste_ws <- gsub("\\.xml$","",liste_f_xml_maitre)
# Noms des workspaces a traiter 
liste_ws <- "./WS/WS_fixed_model.xml"

# Attention cruncher : si on ne veut rien toucher aux specs definies : policy = parameters 
for (ws in liste_ws) {
   print(ws)
   cruncher_and_param(workspace = sprintf("%s.xml", ws),
